...
var resource = {
	"foo": {
		"name": "foo's Protected Resource",
		"description": "This data has been protected by OAuth 2.0"
	},
	"bar": {
		"name": "bar's Protected Resource",
		"description": "This data has been protected by OAuth 2.0"
	}
};
...
var getResource = function(scope){
	var _resource = {}
	scope.forEach(item => {
		_resource[item] = resource[item]
...
app.post("/resource", cors(), getAccessToken, function(req, res){
	...
	if (req.access_token) {
		var _resource = getResource(req.access_token.scope);
		res.json(_resource);
	} else {
		res.status(401).end();
	}
